 ...
 * VERSÃO: 4.0.0 (Análise de Tarefa Única)
 * * DESCRIÇÃO: 
 * Esta versão foca em analisar profundamente uma única tarefa por vez.
 * O sistema coleta um número definido de amostras do tempo de execução (CT) 
 * da tarefa especificada e, ao final, envia os dados brutos e um resumo 
 * estatístico (Min, Max, Média) pelo terminal.
 */
...
// =============================================================================
// 1. Defina o ID de cada tarefa que pode ser analisada
typedef enum {
    TASK_ID_RPM,
    TASK_ID_VELOCIDADE,
    TASK_ID_TEMPERATURA,
...
// 2. ESCOLHA QUAL TAREFA ANALISAR AQUI
#define TASK_TO_ANALYZE TASK_ID_VELOCIDADE

// 3. Defina o número de amostras a serem coletadas
...
// =============================================================================
// Vetor para armazenar as amostras de tempo de execução da tarefa selecionada
uint32_t g_execution_samples[MAX_SAMPLES];
// Contador de quantas amostras já foram coletadas
...
// =============================================================================
void task_analysis_report(void *params) {
    // Aguarda o gatilho, que agora é enviado pela tarefa que está sendo analisada
    xSemaphoreTake(g_sem_report_trigger, portMAX_DELAY);

    printf("\n\n---====== RELATORIO DE AMOSTRAS DE TEMPO DE EXECUCAO (CT) ======---\n");
    printf("Coleta de %d amostras concluida.\n\n", MAX_SAMPLES);
    
    // Imprime o vetor completo de amostras
    printf("--- Dados Brutos (us) ---\n");
    for (int i = 0; i < MAX_SAMPLES; i++) {
        printf("%lu ", g_execution_samples[i]);
        if ((i + 1) % 20 == 0) { // Quebra de linha a cada 20 amostras
            printf("\n");
        }
    }

    // Calcula e imprime as estatísticas
    uint32_t min_time = g_execution_samples[0];
    uint32_t max_time = g_execution_samples[0];
    uint64_t sum_time = 0;
    for (int i = 0; i < MAX_SAMPLES; i++) {
        sum_time += g_execution_samples[i];
        if (g_execution_samples[i] < min_time) min_time = g_execution_samples[i];
        if (g_execution_samples[i] > max_time) max_time = g_execution_samples[i];
    }
    float avg_time = (float)sum_time / MAX_SAMPLES;

    printf("\n\n--- Resumo Estatistico ---\n");
    printf("Tempo Minimo (Best Case): %lu us\n", min_time);
    printf("Tempo Maximo (Worst Case): %lu us\n", max_time);
...
// Função auxiliar para registrar uma amostra de tempo.
void record_sample(uint32_t ct) {
    // Acesso à variável global g_sample_count deve ser atômico
    taskENTER_CRITICAL();
    if (g_sample_count < MAX_SAMPLES) {
        g_execution_samples[g_sample_count] = ct;
        g_sample_count++;
        // Se o buffer estiver cheio, dispara o relatório
        if (g_sample_count == MAX_SAMPLES) {
            // Não é ideal dar o semáforo dentro de uma seção crítica,
            // mas como o sistema vai parar logo em seguida, é aceitável aqui.
            xSemaphoreGive(g_sem_report_trigger);
        }
    }
    taskEXIT_CRITICAL();
}
...
void task_logica_airbag(void *params) {
    while (true) {
        ...
        if (TASK_TO_ANALYZE == TASK_ID_AIRBAG) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_logica_abs(void *params) {
    while (true) {
        ...
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_ABS) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_comando_piloto(void *params){
    while (true) {
        ...
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_PILOTO) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_debounce_buttons(void *params){
    ...
    while(true){
        ...
        switch(s){case R:if(pr){s=P;c=1;}break;case P:if(pr){c++;if(c>=D){xSemaphoreGive(g_sem_piloto);s=H;}}else{s=R;}break;case H:if(!pr){s=R;}break;}
        
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_DEBOUNCE) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_monitor_rpm(void *params){
    ...
    while (true) {
        ...
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_RPM) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_monitor_velocidade(void *params){
    ...
    while (true) {
        ...
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_VELOCIDADE) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_monitor_temperatura(void *params){
    ...
    while(true){
        ...
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_TEMPERATURA) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_monitor_combustivel(void *params) {
    ...
    while (true) {
        ...
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_COMBUSTIVEL) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void task_gerencia_display(void *params){
    ...
    while(true){
        ...
        uint64_t end_time = time_us_64();
        if (TASK_TO_ANALYZE == TASK_ID_DISPLAY) {
            record_sample(end_time - start_ct);
        }
    }
}
...
void gpio_callback_isr(uint gpio, uint32_t events){ 
    ...
    if(gpio==PIN_SENSOR_COLISAO)xSemaphoreGiveFromISR(g_sem_airbag,&woken);
    else if(gpio==PIN_SENSOR_ABS)xSemaphoreGiveFromISR(g_sem_abs,&woken);
...
int main() {
    ...
    printf("\n--== STR Veicular com FreeRTOS (v4.0.0 - Analise de Tarefa Unica) ==--\n");
    printf("Analisando Tarefa ID: %d. Coletando %d amostras...\n", TASK_TO_ANALYZE, MAX_SAMPLES);
